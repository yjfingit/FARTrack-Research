...
reproducible protocol.  The held-out baseline-only LaSOT run is in progress at
the time of this draft and its result is intentionally not reported here.
\end{abstract}
...
\section{Protocol}
\paragraph{Model and invariants.} Every experiment uses the released
...
\paragraph{Development/test separation.} B\textsubscript{dev} is the public
...
survived B\textsubscript{dev}.

\paragraph{Statistics and speed.} We report the mean of per-frame IoUs. For a
...
positive method. The LaSOT test score is pending and must not be inferred from
GOT-10k validation. GOT-10k test server submission has not been performed.
