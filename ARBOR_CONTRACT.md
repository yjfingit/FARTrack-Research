...
- **Metric**: maximize LaSOT AUC on a fixed development subset. Full LaSOT and GOT-10k test are held out until configuration freeze.
- **Baseline**: to be measured using FARTrackSparse ep0015 with identical checkpoint and protocol.
...
- **Budget**: real GPU experiments; start with short development sequences, then one fixed dev subset. No model training.
